...
def 効果名を英語で返す(列名: str) -> str:
    対応 = {
        "group": "Group",
        "condition": "Condition",
        "time": "Time",
    }
    return 対応.get(列名, str(列名))


def 主効果タイトルを作る(列名: str) -> str:
    return f"Mean value by {効果名を英語で返す(列名)}"


def 交互作用タイトルを作る(*列名一覧: str) -> str:
    return " × ".join(効果名を英語で返す(列名) for 列名 in 列名一覧)
...
def 三要因プロットを作る(長形式データ: pd.DataFrame, 系列ラベル対応: dict[str, str]):
    ...
    return 交互作用プロットを作る(
        ...
        x列="time",
        系列列="系列",
        タイトル=交互作用タイトルを作る("group", "condition", "time"),
        x軸ラベル="Time",
        ...
        with タブ:
            ...
            if 項 == "群":
                ...
                fig = 交互作用プロットを作る(
                    ...
                    x列="group",
                    系列列="condition",
                    タイトル=交互作用タイトルを作る("group", "condition"),
                    x軸ラベル="Group",
                    凡例タイトル="Condition",
                    xラベル対応=群コード対応,
                    系列ラベル対応=条件コード対応,
                ...
                fig = 交互作用プロットを作る(
                    ...
                    x列="time",
                    系列列="condition",
                    タイトル=交互作用タイトルを作る("condition", "time"),
                    x軸ラベル="Time",
                    凡例タイトル="Condition",
                    xラベル対応=時間コード対応,
                    系列ラベル対応=条件コード対応,
